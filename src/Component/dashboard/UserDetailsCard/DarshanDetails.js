...
const DarshanDetails = () => {
  ...
  const handlePrint = () => {
    const actionColIndex = 7; // placeholder (no action col, kept same)
    ...
    newWindow.document.write(`
      <html>
      <head>
        <title>Darshan Booking List</title>
        <style>
  ...
  //  EXCEL DOWNLOAD FUNCTION (styled, no Action col)
  const handleDownload = () => {
    ...
    const data = filteredData.map((booking, index) => ({
      "S.No": index + 1,
      "Full Name": booking.full_name,
      "Mobile Number": booking.mobile_number,
      "ID Proof": booking.id_proof_type
        ? `${booking.id_proof_type}: ${booking.id_proof_number}`
        : "—",
      "Temple Name": booking.temple_name,
      "Booking Date & Time": booking.book_date_and_time
        ? new Date(booking.book_date_and_time).toLocaleString("en-IN", {
          dateStyle: "medium",
          timeStyle: "short",
        })
        : "—",
      ...
      "Grand Total": booking.grand_total ? `₹${booking.grand_total}` : "—",
    }));
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Darshan Bookings");
    const wbout = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    saveAs(
      new Blob([wbout], { type: "application/octet-stream" }),
      "Darshan_Booking_List.xlsx"
    );
  };
