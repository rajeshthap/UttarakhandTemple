...
const AcceptedBooking = () => {
  ...
  const handlePrint = () => {
    ...
    newWindow.document.write(`
      <html>
      <head>
        <title>Accepted Booking List</title>
        <style>
  ...
  //  Excel Download (styled, no Action column)
  const handleDownload = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Accepted Bookings");

    const wbout = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    saveAs(new Blob([wbout], { type: "application/octet-stream" }), "Accepted_Booking_List.xlsx");
  };
