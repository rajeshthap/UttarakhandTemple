...
const MandirBooking = () => {
  ...
  useEffect(() => {
    const fetchPoojas = async () => {
      try {
        ...
        if (res.data && Array.isArray(res.data.poojas)) {
          const formatted = res.data.poojas.map((p) => ({
          ...
          // Optional: If you also need the temple name
          if (res.data.temple_name) {
            setFormData((prev) => ({
              ...prev,
              temple_name: res.data.temple_name,
            }));
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const handlePoojaChange = (selected) => {
        ...
        setFormData((prev) => ({
          ...prev,
          pooja_name: selected.map((p) => p.name).join(", "),
            ...
            <Row>
              ...
              <Col lg={6} md={6} sm={12}>
                <Form.Group className="mb-3" controlId="poojaName">
                  <Form.Label className="temp-label">
                    Pooja Name <span className="temp-span-star">*</span>
                  </Form.Label>
